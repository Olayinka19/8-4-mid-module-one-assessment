...
function filterByCountMinimum(animals, minimum) {
  ...
  for(i = 0; i < animals.length; i++) {
    if (animals[i].ccount >= minimum) {
      minValReached.push(animals[i])
    }
  }
  return minValReached;
}
...
function getMostCommonAnimal(animals) {
 let max = null;
 for (let i = animals.length-1; i--;){
   let largest = animals[0];
   if(animals[i] > largest) {
     largest = animals[i];
     max = largest;
   }
   return max;
